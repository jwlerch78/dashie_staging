...
// Add these methods to your existing SettingsController class in js/settings/settings-controller.js

// Method for restoring settings (for cancel functionality)
restoreSettings(originalSettings) {
  this.currentSettings = { ...originalSettings };
  this.isDirty = true; // Mark as dirty to ensure proper state
  console.log('⚙️ ↩️ Settings restored to previous state');
  
  // Notify UI components of the restoration
  this.notifyUIUpdate();
}

// Method to get a complete copy for backup
getSettingsSnapshot() {
...
// Enhanced setSetting that tracks changes
setSetting(path, value) {
  if (!this.isInitialized) {
    console.warn('⚙️ Settings not initialized, cannot set:', path);
    return false;
...
// Force save settings (for save button)
async forceSave() {
  try {
    console.log('⚙️ 💾 Force saving settings...');
    await this.storage?.saveSettings(this.currentSettings);
    this.isDirty = false;
    console.log('⚙️ ✅ Settings force saved successfully');
    return true;
  } catch (error) {
    console.error('⚙️ ❌ Failed to force save settings:', error);
    return false;
  }
}
